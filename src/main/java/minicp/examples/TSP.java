...
#choice: 3234908
    ...
    public static Choice tspHeuristic(IntVar[] x, int[][] matrix) {
        ...
        class TSPVariableSelector implements VariableSelector<IntVar>, VariableFilter<IntVar>, VariableEvaluator<IntVar> {
        ...
        class TSPValueSelector implements ValueSelector {

            @Override
            public int getValue(IntVar[] x, int index) {
                int[] domain = new int[x.length];
                int domainSize = x[index].fillArray(domain);

                int min = Integer.MAX_VALUE;
                int closest = domain[0];
                for (int j = 0; j < domainSize; ++j) {
                    int distance = matrix[index][domain[j]];
                    if (distance < min) {
                        min = distance;
                        closest = domain[j];
                    }
        ...
        FirstFailBranching branching = new FirstFailBranching(x);
        branching.setVariableSelector(new TSPVariableSelector());
        branching.setValueSelector(new TSPValueSelector());
